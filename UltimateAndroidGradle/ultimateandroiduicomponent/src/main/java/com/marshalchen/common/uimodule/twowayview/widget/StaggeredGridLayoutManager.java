...
import com.marshalchen.common.uimodule.twowayview.TwoWayLayoutManager;
...
public class StaggeredGridLayoutManager extends GridLayoutManager {
    ...
    public StaggeredGridLayoutManager(TwoWayLayoutManager.Orientation orientation, int numColumns, int numRows) {
    ...
    @Override
    void getLaneForPosition(LaneInfo outInfo, int position, TwoWayLayoutManager.Direction direction) {
        final StaggeredItemEntry entry = (StaggeredItemEntry) getItemEntryForPosition(position);
        if (entry != null) {
    ...
    @Override
    void getLaneForChild(LaneInfo outInfo, View child, TwoWayLayoutManager.Direction direction) {
        ...
        for (int i = 0; i <= position; i++) {
            ...
            if (entry != null) {
                ...
                lanes.getChildFrame(mTempRect, entry.width, entry.height, mTempLaneInfo,
                        TwoWayLayoutManager.Direction.END);
            } else {
                ...
                lanes.getChildFrame(mTempRect, getDecoratedMeasuredWidth(child),
                        getDecoratedMeasuredHeight(child), mTempLaneInfo, TwoWayLayoutManager.Direction.END);
    ...
    @Override
    ItemEntry cacheChildLaneAndSpan(View child, TwoWayLayoutManager.Direction direction) {
